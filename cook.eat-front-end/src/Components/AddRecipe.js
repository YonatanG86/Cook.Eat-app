...
const AddRecipe = (props) => {
	...
	const saveIngredient = () => {
		if (steps.length === 0 || ingredients.length === 0) {
			...
			formInfo.ingredients = ingredients.filter((item) => Object.keys(item).length !== 0);
	...
	return (
		<div className='add-recipe-form-container'>
			...
			<Form validated className='recipe-form' onSubmit={handleSubmit(onSubmit)}>
				...
				<Button className='add-ingredient-btn' type='button' onClick={addIngredient}>
					Add ingredient
				...
				<Button className='add-ingredient-btn steps-btn' type='button' onClick={addStep}>
					Add steps
					<img src='../addRecipe/plus.png' alt='+' />
				</Button>
				...
				{!update ? (
					<Button name='Add' className='add-recipe-btn' type='submit' disabled={disabled}>
						Submit your Recipe
					</Button>
				) : (
					<Button name='Updated' className='add-recipe-btn' type='submit' disabled={disabled}>
						Update Recipe
					</Button>
				)}
			</Form>
			<img className='add-recipe-img' src='../addRecipe/add_recipe.jpg' alt='add-recipe' />
			<BounceLoader css={override} size={80} color={'#B73032'} loading={loading} />
		</div>
	);
};
